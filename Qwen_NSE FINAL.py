...
Automated NSE High-Delivery Scanner.
Fixes:
 - Deduplicate database rows per (SYMBOL, DATE) before analysis,
   keeping the row with the largest DELIV_QTY.
 - Ensure final output contains only one row per SYMBOL for today's snapshot.
Other features:
 - Robust CSV reading + normalization
 - Stores cleaned SERIES == 'EQ' rows into SQLite
 - Computes average delivery over five most recent distinct prior dates (DATE < target_date)
 - Filters: DELIV_QTY >= 10_000, %CHANGE > 0, DELIVERY_TIMES >= 3
 - Outputs CSV and log; optional Telegram upload
 - **ADDED**: Enriches final output with Market Cap from Screener.in and filters for >= ₹100 Cr.
"""
import argparse
import logging
import os
...
def create_daily_snapshot_for_date(db_path: str, target_date, logger=None) -> pd.DataFrame:
    ...
    cols = ["SYMBOL", "DATE", "PREV_CLOSE", "CLOSE_PRICE", "DELIV_QTY", "DELIV_PER", "%CHANGE", "AVG_DELIV_QTY_5D_PRIOR", "DELIVERY_TIMES"]
    for c in cols:
...
def enrich_with_market_cap(input_csv_path: str, logger=None) -> str:
    ...
    for index, row in df.iterrows():
        ...
        tradingview_url = f"https://in.tradingview.com/chart/ob4F1XSN/?symbol=NSE:{symbol}/"
        # Fetch market cap
